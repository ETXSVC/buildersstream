...
# Start all services (frontend starts automatically via docker-start.sh)
docker compose up -d
...
**After editing frontend files on Windows**, restart the frontend container immediately:
```bash
docker compose up -d --force-recreate frontend
```
Vite uses polling (`usePolling: true, interval: 500ms`) on Windows Docker volumes. After any file edit, the polling watcher detects the file as continuously "changed" (NTFS/WSL2 mtime discrepancy) and sends full-reload signals in a loop. Restarting resets the watcher's mtime baseline — solves the reload loop without touching any files.

**Files that trigger Vite behavior when edited:**
- `vite.config.ts` → Vite server restart (then full browser reload)
- `index.html` → Vite full browser reload on next poll
- `.env*` files → Vite server restart
- Files in `public/` → Vite full browser reload on next poll

In all cases: edit the file, then immediately `docker compose up -d --force-recreate frontend`.
...
- **Vite polling loop on Windows Docker volumes**: Vite's `usePolling: true` watcher detects recently-edited files as continuously changed due to NTFS/WSL2 mtime differences. Symptom: browser reloads every ~500ms, backend logs show repeated 200 responses for the same user. Fix: `docker compose up -d --force-recreate frontend` immediately after any frontend file edit. This resets Vite's watcher baseline.
- **`request.build_absolute_uri()` in Docker**: Using it for media URLs produces `http://web:8000/media/...` (Docker-internal hostname via the `Host` header from Vite proxy). Browser cannot resolve `web`. Always use `obj.file_field.url` directly — returns the relative `/media/...` path.
- **Build timestamp pattern**: Use `transformIndexHtml` plugin in `vite.config.ts` to inject `window.__BUILD_TIME__`. Never write to `public/`, `.env*` files, or any file at startup — all trigger Vite's polling false-positive loop.
- **`refetchOnWindowFocus`**: Set to `false` on all React Query hooks by default (global config in `main.tsx`). Per-hook overrides should also use `false` unless actively required.
...
The platform is feature-complete at v3.4. All 19 Django apps, 4 sprints, and 74 E2E tests are done.
...
- Upload branding logo/favicon via Django admin or Settings → Branding to populate `/media/branding/` (currently 404 since media directory is empty)
- PWA icons (`/icons/icon-192.png`, `/icons/icon-512.png`) missing — add to `frontend/public/icons/`
