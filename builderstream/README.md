...
The `frontend` service starts automatically via Docker (node:20 image, `docker-start.sh`). No local `npm install` needed.
...
**After editing frontend files on Windows**, restart the frontend container to reset Vite's file-watcher baseline and prevent false-positive HMR reload loops:
```bash
...
### v3.4.0 (2026-05-28)
- **Build timestamp**: Nav bar center column shows ISO build time (when container started) via Vite `transformIndexHtml` plugin — no file writes, no polling false-positives
- **Media URLs**: `BrandingView` now returns relative `/media/...` paths instead of `request.build_absolute_uri()`, which was producing `http://web:8000/media/...` (Docker-internal, not browser-resolvable). Vite proxy now forwards `/media` to Django
- **Dashboard stability**: `useDashboard.ts` changed `refetchOnWindowFocus: true → false` to stop dashboard re-fetching on tab switch
- **Vite proxy**: Added `/media` proxy entry in `vite.config.ts` (alongside existing `/api`, `/ws`, `/admin`)
- **Docker-first dev**: `docker-start.sh` is the canonical frontend start — runs `npm install` then Vite dev server inside container; no local node required
- **TypeScript**: Added `"ignoreDeprecations": "5.0"` to `tsconfig.json` to silence `baseUrl` warning
